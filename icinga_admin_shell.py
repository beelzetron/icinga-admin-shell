...
class IcingaAdminShell(cmd.Cmd):
    ...
    def do_create(self, line):
        if line == 'host':
            while True:
                ...
                if self._is_valid_input(os, ['windows', 'linux']):
                    break
                else:
                    continue
            while True:
                env = raw_input('Role [prod|svil]: ')
                if self._is_valid_input(env, ['prod', 'svil']):
                    break
                else:
                    continue
            ...
            if not address:
                address = _address
            try:
                Model.Host.objects.get_by_shortname(hostname)
    ...
    def complete_showhost(self, text, line, begidx, endidx):
        ...
        for h in self._all_hosts:
            if h.get_shortname is not None:
                hosts.append(h.get_shortname())
        if not text:
            completions = hosts[:]
        else:
            completions = [f
